...
func TestClientCreateLaptop(t *testing.T) {
	...
	require.Equal(t, laptop.Id, expectedID)
...
func TestClientSearchLaptop(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		switch i {
		case 0:
			laptop.PriceInr = 1500000
		case 1:
			laptop.Cpu.NumberOfCores = 2
		case 2:
			laptop.Cpu.MinGhz = 1.5
		case 3:
			laptop.Ram = &pb.Memory{Value: 4, Unit: pb.Memory_GIGYBYTE}
		case 4:
			laptop.PriceInr = 99999
			laptop.Cpu.NumberOfCores = 4
			laptop.Cpu.MinGhz = 2.5
			laptop.Cpu.MaxGhz = 4.5
			laptop.Ram = &pb.Memory{Value: 12, Unit: pb.Memory_GIGYBYTE}
			expectedIDs[laptop.Id] = true
		case 5:
			laptop.PriceInr = 60000
...
func TestClientRateLaptop(t *testing.T) {
	...
	scores := []float64{5, 7, 9}
	averages := []float64{5, 6, 7}
